...
// //       setFilteredInterns(response.items);
...
// //   return (
// //     <div>
// //       <h2 className="text-xl font-bold mb-4">Intern Performance</h2>
...
// //               <td>{intern.name}</td>
// //               <td>{intern.email}</td>
// //               <td>{intern.internType?.toUpperCase()}</td>
// //               <td>{intern.joinedDate?.slice(0, 10)}</td>
// //               <td>
...
//         limit: 200
//       });
//       const internsData = response.items || response || [];
...
//       // Sort by completion date (recent first)
...
//                           const notStarted = internTasks.filter(t => t.status === "NOT_STARTED").length;
...
//                         {(() => {
//                           const completedTasks = internTasks.filter(t => t.status === "DONE" && t.completedAt && t.createdAt);
...
//                       <Button onClick={handleExportReport} size="sm" variant="outline" className="flex items-center gap-2">
...
//                               <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${getTaskStatusColor(task.status)}`}>
...
//                                     ⏱️ {Math.ceil((new Date(task.completedAt).getTime() - new Date(task.createdAt).getTime()) / (1000 * 60 * 60 * 24))} days
//                                   </span>
//                                 </>
//                               )}
//                             </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
            <div className="bg-white rounded-2xl shadow-2xl max-w-6xl w-full my-8">
              ...
              <div className="p-6 space-y-6 max-h-[calc(100vh-16rem)] overflow-y-auto">
                ...
                {/* Performance Metrics and Task Details... */}
              </div>
            </div>
          </div>
        )}
