...
@app.post("/api/v1/auth/sso/azure/callback")
async def azure_sso_callback(request: dict, db = Depends(get_database)):
    ...
    - new_user: User needs to complete profile
    - pending_approval: User exists but not approved yet
    - approved: User is approved and can proceed to dashboard
    """
    from sso_handler import handle_azure_sso_callback

    code = request.get("code")
    if not code:
        raise HTTPException(status_code=400, detail="Missing authorization code")

    return await handle_azure_sso_callback(code, db)


@app.post("/api/v1/auth/sso/complete-profile")
async def complete_profile(request: dict, db = Depends(get_database)):
    """
    Complete profile for new SSO user after profile completion form.
    Expects profile data including personal and internship details.
    """
    from sso_handler import complete_sso_profile

    return await complete_sso_profile(request, db)
...
@app.get("/api/v1/users/me/mentor")
async def get_my_mentor(
    current_user: User = Depends(get_current_active_user),
    db = Depends(get_database)
):
    """
    Get mentor details for the current user (intern or scrum master)
    Returns mentor info, mentees list, and active projects
    """
    try:
        # Find the user's intern/profile record to get mentor name
        user_email = current_user.email.lower()
        intern_record = await db.users.find_one({"email": user_email})
        ...
        mentor_name = intern_record.get("mentor")
        ...
        # Find mentor's user record
        mentor_user = await db.users.find_one({"name": mentor_name})
        ...
        # Find all mentees (interns with this mentor)
        mentees = []
        async for user in db.users.find({"mentor": mentor_name, "role": "intern"}):
            mentees.append({
                "name": user.get("name", ""),
        ...
        # Find active projects where this mentor is involved
        projects = []
        async for proj in db.projects.find({"mentor": mentor_name, "status": "active"}):
            projects.append({
                "name": proj.get("name", ""),
        ...
        return {
            "mentor": {
                "name": mentor_user.get("name", mentor_name),
                "email": mentor_user.get("email", ""),
                "employeeId": mentor_user.get("employee_id", ""),
                "role": mentor_user.get("role", "scrum_master").replace("_", " ").title()
            },
            "mentees": mentees,
            "projects": projects
        }
        ...
        print(f"[Mentor API] Error fetching mentor details: {e}")
        raise HTTPException(
            status_code=500,
...
async def get_notifications(
    ...
    """
    try:
        # TODO: Implement actual notification collection in database
        # For now, return empty list to prevent UI errors
        return {
            "notifications": [],
            "unreadCount": 0,
            "total": 0,
...
async def mark_notification_read(
    ...
    # TODO: Implement when notifications collection is added
...
@app.delete("/api/v1/notifications/clear-read")
...
async def get_intern(
    ...
    """Get intern by ID with calculated metrics"""
    # Query users collection (interns are users with role=intern or scrum_master)
    intern = await db.users.find_one({"_id": ObjectId(intern_id)})
    if not intern:
        raise HTTPException(status_code=404, detail="Intern not found")

    # Calculate real-time metrics
    intern_id_str = str(intern["_id"])

    # Count total tasks
    total_tasks = await db.tasks.count_documents({"internId": intern_id_str})

    # Count completed tasks (status = 'completed')
    completed_tasks = await db.tasks.count_documents({
        "internId": intern_id_str,
        "status": "completed"
    })

    # Calculate DSU streak (consecutive days with submitted DSUs)
    dsu_streak = await calculate_dsu_streak(db, intern_id_str)

    # Update calculated fields
    intern["taskCount"] = total_tasks
    intern["completedTasks"] = completed_tasks
    intern["dsuStreak"] = dsu_streak
    intern["_id"] = intern_id_str
...
async def create_project(
    ...
    # Convert datetime.date → datetime.datetime so BSON/PyMongo can encode them
    for field in ("startDate", "endDate"):
        val = project_dict.get(field)
        if isinstance(val, date) and not isinstance(val, datetime):
            project_dict[field] = datetime(val.year, val.month, val.day, tzinfo=timezone.utc)
...
async def update_project(
    ...
    for field in ("startDate", "endDate"):
        ...
        if isinstance(val, date) and not isinstance(val, datetime):
            update_data[field] = datetime(val.year, val.month, val.day, tzinfo=timezone.utc)
