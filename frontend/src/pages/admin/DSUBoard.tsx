...
const DSUBoard: React.FC = () => {
  ...
  const styles: Record<string, React.CSSProperties> = {
    ...
    title: { fontSize: '28px', fontWeight: 800, color: '#1e1145', margin: 0 },
    statsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '32px' },
    statCard: { background: 'white', padding: '20px', borderRadius: '16px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', gap: '16px', borderLeft: '4px solid #6366f1' },
  ...
  if (loading) return <div style={{ display: 'flex', height: '100vh', alignItems: 'center', justifyContent: 'center', background: '#f8fafc' }}><RefreshCw className="animate-spin" size={40} color="#6366f1"/></div>;
    ...
    <div style={styles.container}>
      ...
      <header style={styles.header}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          <button style={{ padding: '10px 20px', borderRadius: '8px', background: '#1e1145', color: 'white', border: 'none', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      <div style={styles.statsGrid}>
        <div style={styles.statCard}>
          <div style={{ background: '#eff6ff', padding: '12px', borderRadius: '12px' }}><Users color="#3b82f6"/></div>
          ...
          <div key={intern._id} style={styles.internCard}>
            <div style={styles.cardHeader}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ width: '40px', height: '40px', background: '#6366f1', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700 }}>
                  {intern.name[0]}
                </div>
                <div>
              ...
              <div style={{ textAlign: 'right' }}>
                <span style={{ fontSize: '12px', fontWeight: 700, color: '#6366f1' }}>{internTasks.length} UPDATES</span>
              </div>
            </div>
