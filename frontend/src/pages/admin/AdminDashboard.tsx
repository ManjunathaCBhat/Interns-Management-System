...
const AdminDashboard: React.FC = () => {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 font-sans">
      {/* Sidebar */}
      <aside className={`${sidebarOpen ? 'w-60' : 'w-0'} transition-all duration-300 overflow-hidden bg-gradient-to-b from-[#1e1145] to-[#2d1b69] text-white flex flex-col`}>
        <div className="p-6 border-b border-white/10 font-bold text-xl">cirruslabs</div>
        <nav className="flex-1 p-4 space-y-2">
          {[
            ...
            <Link key={item.id} to={item.path} className={`flex items-center gap-3 p-3 rounded-lg hover:bg-white/10 ${location.pathname === item.path ? 'bg-purple-500/30' : ''}`}>
              <item.icon size={18} /> {item.label}
            </Link>
          ))}
        </nav>
        <button className="p-6 flex items-center gap-3 text-white/50 hover:text-white border-top border-white/10">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white border-b p-4 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-4">
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 bg-[#1e1145] text-white rounded-lg"><Menu size={20}/></button>
