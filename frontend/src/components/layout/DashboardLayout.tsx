...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children }) => {
    ...
    return (
      <Link
        ...
        className={cn(
          ...
          isActive
            ? 'bg-accent-500/80 text-white shadow-pink'
            : 'text-sidebar-foreground/70 hover:bg-accent-500/10 hover:text-sidebar-foreground'
        )}
      >
  ...
  return (
    <div className="flex min-h-screen w-full bg-background">
      ...
      >
        {/* Logo & Toggle - WITH HOVER AND ACTIVE STATES */}
        <div className="flex h-16 items-center justify-between border-b border-sidebar-border px-4">
          <Link 
            to={getHomeRoute()} 
            className={cn(
              "flex items-center gap-2 px-3 py-2 rounded-lg transition-all duration-300",
              "hover:bg-accent-500/10 hover:shadow-lg hover:shadow-accent-500/20",
              "active:bg-accent-500/20 active:scale-95 active:shadow-inner",
              "cursor-pointer"
            )}
          >
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-primary transition-transform hover:scale-110" style={{ backgroundColor: '#3D2F6F' }}>
              <span className="text-lg font-bold text-white">
                I
              </span>
            </div>
            {sidebarOpen && (
              <span className="font-semibold text-sidebar-foreground transition-colors hover:text-accent-500">
                Interns360
              </span>
            )}
          </Link>
          <button
            ...
            className="rounded-lg p-1.5 text-sidebar-foreground/70 hover:bg-accent-500/10 hover:text-accent-500 transition-all"
          >
            <ChevronLeft
              className={cn(
      ...
      <div className="fixed left-0 right-0 top-0 z-50 flex h-14 items-center justify-between border-b bg-background px-4 md:hidden">
        <Link 
          ...
          className={cn(
            "flex items-center gap-2 px-3 py-2 rounded-lg transition-all duration-300",
            "hover:bg-accent-500/10 active:bg-accent-500/20 active:scale-95"
          )}
        >
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-primary transition-transform hover:scale-110">
            <span className="text-sm font-bold text-white">IL</span>
          </div>
          <span className="font-semibold transition-colors hover:text-accent-500">Interns360</span>
        </Link>
        <button
          onClick={() => setMobileOpen(!mobileOpen)}
          className="rounded-lg p-2 hover:bg-accent-500/10 hover:text-accent-500 transition-all"
        >
            ...
            <div className="absolute bottom-0 left-0 right-0 border-t border-sidebar-border p-3">
              <div className="flex items-center justify-between rounded-lg px-3 py-2">
                ...
                <Button
                  ...
                  size="icon"
                  onClick={handleLogout}
                  className="text-sidebar-foreground/70 hover:text-accent-500 hover:bg-accent-500/10 transition-all"
                >
                  <LogOut className="h-4 w-4" />
                </Button>
              </div>
